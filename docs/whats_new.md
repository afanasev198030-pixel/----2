...
## 2026-03-13 — Логика граф 8, 9, 14: трёхсторонний договор и приоритет источников

### Графы 8 и 9 — единственный источник: трёхсторонний договор

Раньше: графы «Получатель» и «Лицо, ответственное за финансовое урегулирование» заполнялись из контракта, инвойса или профиля компании. Из-за этого в них дублировались полные реквизиты декларанта, хотя по правилам таможенного оформления при стандартной двусторонней сделке должно быть «СМ. ГРАФУ 14 ДТ».

Теперь:
- Система проверяет наличие **трёхстороннего договора** среди загруженных документов.
- Если трёхстороннего договора **нет** → графы 8 и 9 заполняются текстом **«СМ. ГРАФУ 14 ДТ»**.
- Если трёхсторонний договор **есть** → из него извлекаются данные получателя (гр. 8) и финансово ответственного лица (гр. 9): наименование, адрес, ИНН/КПП, ОГРН.
- Другие источники (профиль компании, контракт, инвойс, доверенность, договор с брокером) **убраны** из логики этих граф.

### Графа 14 — новый приоритет источников

Раньше: данные декларанта брались в первую очередь из профиля компании в системе, а контракт/инвойс использовался как fallback.

Теперь:
- Приоритет изменён: **контракт** (1) → **инвойс на товары** (2) → **профиль компании** (3).
- Каждое поле (наименование, адрес, ИНН, КПП, ОГРН) проверяется отдельно — берётся первое непустое значение в порядке приоритета.
- Добавлена **языковая проверка**: если контракт содержит название на латинице (AG-LOGISTIK LLC), а профиль компании — на кириллице (ООО «АГ-ЛОГИСТИК»), для имени и адреса предпочитается **русский вариант**, поскольку ДТ заполняется на русском языке.
- ИНН/КПП/ОГРН всегда берутся по прямому приоритету (контракт → инвойс → профиль), поскольку это цифровые данные без языковой привязки.

### Отображение «СМ. ГРАФУ 14 ДТ» в PDF и на фронтенде

Раньше: даже когда получатель совпадал с декларантом (один и тот же ID контрагента), система отображала полные реквизиты компании в графах 8 и 9 — и в PDF-экспорте, и на странице просмотра.

Теперь:
- PDF-экспорт (`export_pdf.py`): если `receiver_counterparty_id == declarant_counterparty_id` → выводится «СМ. ГРАФУ 14 ДТ» (основной и добавочные листы).
- Фронтенд (`DeclarationViewPage.tsx`): аналогичная проверка для граф 8 и 9.
- XML-экспорт уже обрабатывал это корректно (`EqualIndicator: true`), изменения не потребовались.

### Регистрация с привязкой к компании

- В форму регистрации добавлено поле «Название компании» — новый пользователь сразу получает привязку к компании.
- На странице деклараций: если аккаунт не привязан к компании — кнопка создания скрыта, отображается пояснение.

### Затронутые файлы

- `declaration_mapping_v3.yaml` — YAML-правила AI для граф 8, 9, 14
- `declaration_ai_filling_rules.md` — документация по заполнению
- `apply_parsed.py` — бэкенд: детекция трёхстороннего договора, приоритет источников, языковая проверка
- `declaration_helpers.py` — хелпер мёрджа ИНН/КПП (приоритет инвертирован)
- `export_pdf.py` — PDF-экспорт: «СМ. ГРАФУ 14 ДТ»
- `DeclarationViewPage.tsx` — фронтенд: «СМ. ГРАФУ 14 ДТ»
- `DeclarationsListPage.tsx` — валидация привязки к компании
- `LandingPage.tsx` — поле компании при регистрации
