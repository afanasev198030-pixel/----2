...
`chat_agent.py` и `agent_tools.py` кэшировали `settings = get_settings()` на уровне модуля — **до** того как при старте подтягивались настройки из БД. В результате агент использовал дефолтную модель `deepseek-chat` через `proxyapi`, который её не знает → HTTP 400.

**Исправление:** все обращения к настройкам теперь через свежий вызов `get_settings()` внутри методов, а не через устаревшую ссылку.

### AI Chat Widget на фронтенде

На все страницы добавлен плавающий виджет AI-ассистента:

| Что | Детали |
|-----|--------|
| Компонент | `ChatWidget.tsx` — FAB-кнопка (правый нижний угол) + окно чата |
| Мобильный | Полноэкранный Dialog с анимацией Slide |
| Десктоп | Плавающее окно 400×520px с тенью |
| Авторизация | Виджет показывается только залогиненным пользователям |
| API | POST `/api/v1/ai/chat` через существующий proxy |
| Сессия | Уникальный `session_id` на вкладку, поддержка истории |
| UX | Enter для отправки, аватары, анимация загрузки, multiline |

### Глубокий System Prompt таможенного эксперта

Системный промпт агента расширен экспертными знаниями:
- Все 54 графы ДТ и зависимости между ними
- ТН ВЭД ЕАЭС (39 000+ кодов), группировки, примечания
- Таможенные процедуры: ИМ40, ЭК10, транзит
- Инкотермс 2020: EXW → DDP, морские FAS/FOB/CFR/CIF
- Таможенная стоимость: методы 1-6, ДТС-1/ДТС-2
- Документы: инвойс, CMR, коносамент, сертификаты
- Правило: на общие вопросы ("что ты умеешь?") отвечает без вызова инструментов

### Telegram-бот на Landing Page

На главную продающую страницу добавлена выделенная секция "AI-ассистент брокера в Telegram":
- 4 блока преимуществ: AI с function calling, документы→декларация, push-уведомления, управление из чата
- Визуальная карточка с имитацией диалога и списком slash-команд
- Карточка "Telegram-бот с AI" в блоке "Возможности"
- Кнопка навигации "Telegram-бот" в хедере
...
- Полностью переписан `chat_agent.py` → `AdvancedConversationalAgent`
- Добавлена поддержка **Claude Opus 4.6** (Anthropic)
- Создан отдельный модуль `agent_tools.py` с набором инструментов
- Добавлена **долгосрочная память** пользователя (Redis, 30 дней)
- Добавлена система **обратной связи** и самообучения (`save_feedback`)
- Улучшен сбор контекста (RAG + декларации + память пользователя)

### Значительно улучшен UX подключения нового пользователя

- Добавлена команда `/start` с понятным приветствием и кнопками
- Добавлена подробная инструкция по привязке аккаунта
- Улучшен обработчик токена с понятными сообщениями успеха и ошибки
- Новый пользователь теперь получает чёткие шаги вместо сухого "привяжите аккаунт"

### Frontend и DevOps

- В `SettingsPage.tsx` добавлен выбор провайдера **Anthropic**
- Обновлены все GitHub Actions workflow для совместимости с Node.js 24
- Добавлена переменная `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`

---


## Рефакторинг agent_crew.py, лист дополнения к ДТ, Cloud.ru embeddings, улучшение парсинга

**Дата:** 2026-03-28 — 2026-04-01
**Ветка:** `feature/provider-profiles-landing`

### Обзор

Крупнейший рефакторинг AI-сервиса: монолитный `agent_crew.py` (4158 строк) разбит на 4 модуля + 4 JSON-справочника, объём сокращён до 555 строк. Добавлен лист дополнения к ДТ (графы 31 и 44 по товарам) во все три представления декларации. Переход на Cloud.ru embeddings (BAAI/bge-m3) для RAG. Улучшена экстракция атрибутов товаров (производитель, марка, модель, артикул, серийные номера). Повторный парсинг теперь корректно заменяет старые позиции и документы.

---

### 1. Рефакторинг agent_crew.py — из 4158 строк в 555

**Проблема**: файл `agent_crew.py` разросся до 4158 строк и содержал ~2000 строк мёртвого кода (legacy-методы компиляции, неиспользуемые классификаторы документов, CrewAI-импорты). Утилитная функция `_safe_float()` дублировалась в 3 файлах с разным поведением. Справочные таблицы (IATA-коды, таможенные посты, коды документов) были вшиты в код — добавление аэропорта требовало правки Python и деплоя. Монолитная `_post_process_compilation()` (670 строк) совмещала 8+ обязанностей.

**Что такое рефакторинг**: реорганизация внутренней структуры кода без изменения его внешнего поведения. Файл делал то же самое, но его невозможно было поддерживать — каждое изменение рисковало задеть мёртвый или не связанный код.

**Решение** — декомпозиция на 4 модуля:

| Модуль | Строк | Назначение |
|---|---|---|
| `declaration_compiler.py` | 234 | LLM-компиляция: промпт + вызов + retry-логика |
| `post_processing.py` | 1241 | Детерминированная пост-обработка: таможенные посты, расчёты, валидация классификаторов, Инкотермс, форматирование |
| `parsing_utils.py` | 127 | Единая реализация `safe_float()`, `normalize_hs_code()`, `to_dict()`, `check_needs_vision_retry()` |
| `reference_data.py` | 81 | Ленивая загрузка JSON-справочников, функции `lookup_customs_office()`, `resolve_iata_city()`, `get_eu_countries()` |

**Удалённый мёртвый код** (~2000 строк):
- `_compile_declaration` (legacy LLM-компиляция, 1000 строк)
- `_compile_by_rules_legacy` (185 строк)
- `_run_crewai` (60 строк) + CrewAI-импорты
- `_batch_parse_secondary` (200 строк)
- `_detect_doc_type` / `_detect_doc_type_debug` / `_classify_invoice_content` (350 строк)
- `_parse_svh_doc`, `_parse_transport_doc_llm` (185 строк)

**Изоляция ошибок в pipeline**: каждый шаг (пост-обработка, HS-классификация, расчёт платежей, оценка рисков, поиск прецедентов) обёрнут в try/except и генерирует issue с `severity: "error"` при сбое, не блокируя остальные шаги.

### 2. JSON-справочники — данные вне кода

**Проблема**: таблицы IATA→таможенный пост (61 запись), AWB-префиксы авиакомпаний (9 записей), коды документов для графы 44, IATA→город (40+ аэропортов), список стран ЕС — всё это было Python-словарями внутри `agent_crew.py`. Для добавления нового аэропорта или таможенного поста нужен был разработчик.

**Решение**: 4 JSON-файла в `services/ai-service/app/data/`:
- `customs_offices.json` — IATA → код таможенного поста + AWB-префиксы → авиакомпании
- `document_codes.json` — коды документов для графы 44 ДТ
- `iata_cities.json` — IATA → название города
- `eu_countries.json` — ISO-коды стран ЕС

Модуль `reference_data.py` загружает их лениво (один раз при первом обращении) и кэширует в памяти.

### 3. Лист дополнения к ДТ

**Проблема**: печатная ДТ содержит до 3 товаров на основном листе, остальные выносятся в «Добавочные листы» (лист дополнения). В нашей системе при наличии нескольких товаров не было отдельного блока с таблицами граф 31 и 44 по каждому товару.

**Что такое лист дополнения**: стандартный раздел таможенной декларации, в котором для каждого товара сверх основного листа указываются: документы (графа 44 — код, номер, дата), описание и характеристики товара (графа 31 — наименование, производитель, марка, модель, количество, единица измерения, артикул, серийные номера).

**Решение** — блок дополнения добавлен во все три представления:
- **DeclarationFormPage** — MUI-таблицы с данными позиций внутри формы по образцу печатной ДТ
- **DeclarationViewPage** — HTML-таблицы в стиле печатной формы (для `@media print`)
- **DeclarationStatusPage** — предупреждения о дрейфе кодов ТН ВЭД и панель требований к документам по каждой позиции

### 4. Cloud.ru embeddings (BAAI/bge-m3)

**Проблема**: RAG-поиск по кодам ТН ВЭД и прецедентам использовал локальные ONNX-модели для эмбеддингов. Качество поиска было ограничено размером модели, а поиск по русскоязычным описаниям товаров часто не находил релевантных кодов.

**Что такое embeddings**: числовые векторы, в которые модель преобразует текст. Похожие по смыслу тексты получают близкие векторы — это позволяет искать «по смыслу», а не по точному совпадению слов. Например, «электрический двигатель» и «мотор» будут иметь близкие векторы.

**Решение**:
- Дефолтный `EMBED_PROVIDER` переключён с `local` на `cloud_ru`
- Добавлена поддержка модели `BAAI/bge-m3` (1024-мерные мультиязычные эмбеддинги) через Cloud.ru Foundation Models
- Новые настройки: `EMBED_MODEL`, `EMBED_BASE_URL`, `EMBED_API_KEY` в config.py и docker-compose.yml
- Очищены RAG-запросы: убраны искусственные префиксы `"Товар: "` и `"query: "` — модель bge-m3 работает без них
- Хранение прецедентов: описания товаров очищаются от шума (номера граф, форматирование) перед векторизацией

### 5. Расширенная экстракция атрибутов товаров

**Проблема**: AI извлекал из документов только description и commercial_name товара. Поля manufacturer, trademark, model_name, article_number, serial_number не заполнялись — брокер вводил их вручную.

**Решение**:
- **Схемы экстракции**: в `llm_parser.py` для типов `tech_description` и `conformity_declaration` добавлены поля: `manufacturer`, `brand`, `model`, `article_number`, `serial_numbers`, `full_description` (полное описание без сокращений)
- **Техописание**: промпт `techop_parser.py` расширен аналогичными полями
- **apply-parsed**: `ParsedItem` в core-api расширен полями manufacturer, trademark, model_name, article_number, serial_number — данные пробрасываются из AI в БД
- **Миграция 031**: поле `commercial_name` расширено с `VARCHAR(500)` до `TEXT` — полное техническое описание товара может превышать 500 символов
- **Типы**: `serial_number` добавлен в TypeScript-интерфейс `DeclarationItem`
- **Debug-логирование**: worker при apply-parsed логирует атрибуты первых 3 товаров для отладки

### 6. Повторный парсинг — корректная замена данных

**Проблема**: при повторном парсинге документов по декларации старые позиции товаров и документы не удалялись. Новые позиции добавлялись к существующим, создавая дубли. Если инвойс заменили на другой, старые и новые товары оказывались в одной декларации.

**Решение**: в `apply_parsed.py` перед созданием новых позиций и документов добавлено удаление старых:
- Позиции (DeclarationItem) и их связи с документами (DeclarationItemDocument) удаляются
- Документы (Document) удаляются
- Удаление происходит в рамках одной транзакции — если создание новых данных упадёт, откатятся и удаления

### 7. Улучшение классификации ТН ВЭД

**Решение** — несколько точечных улучшений:
- **Компоненты дронов**: расширены ключевые слова для видеоприёмников (`видеоприёмник`, `приёма видео`, `5.8GHz`) и видеопередатчиков (`передачи видеосигнал`) — точнее попадание в коды 8526.92 и 8525.60
- **Фильтрация RAG-кандидатов**: отсекаются результаты с `score < 0.40` (сервер) и `< 0.45` (фронт) — убраны нерелевантные подсказки
- **Множественные suggestions**: endpoint `/classify-hs-rag` возвращает до 6 вариантов (основной + кандидаты) вместо одного — брокер видит альтернативы
- **Очистка входного описания**: перед классификацией убирается форматирование графы 31 (номера строк, маркировка)
- **Cloud.ru JSON mode**: включён `supports_json_mode: True` для Cloud.ru — ранее был отключён ошибочно

### 8. Улучшения в StatusPage и FormPage

- **ДТС**: кнопка «Открыть ДТС» показывается только если ДТС уже существует; добавлена кнопка «Сформировать ДТС» с вызовом API и переходом на страницу
- **Загрузка документов**: добавлена панель `DocumentUploadPanel` прямо на StatusPage — можно загрузить новые документы без перехода на другую страницу
- **Следующие шаги**: добавлен компонент `NextActionsPanel` — подсказывает, что делать дальше (загрузить документы, проверить данные, открыть форму)
- **Дрейф кодов**: предупреждения о дрейфе ТН ВЭД отображаются в SourceDrawer (FormPage) и в карточках позиций (StatusPage)
- **Панели анализа**: `RequirementsPanel`, `RiskPanel`, `AiExplainPanel` интегрированы в FormPage SourceDrawer
- **Описание товара**: в ViewPage поле description/commercial_name теперь корректно переносит строки (`white-space: pre-line`)

### 9. Улучшения RAG-индексации

- **Retry с backoff**: при ошибке батча индексации HS-кодов — до 2 повторных попыток с экспоненциальной паузой (5с, 10с)
- **Параметр force**: `POST /init-rag` принимает `force=true` (полная переиндексация) / `false` (дозаполнение)
- **Увеличен timeout**: httpx-клиент индексации — с 300с до 600с для больших батчей

---

### Затронутые файлы

**ai-service — новые модули (4 файла, 1683 строки):**

| Файл | Строк | Назначение |
|---|---|---|
| `services/ai-service/app/services/declaration_compiler.py` | 234 | LLM-компиляция декларации |
| `services/ai-service/app/services/post_processing.py` | 1241 | Детерминированная пост-обработка |
| `services/ai-service/app/services/parsing_utils.py` | 127 | Единые утилиты парсинга |
| `services/ai-service/app/services/reference_data.py` | 81 | Загрузчик JSON-справочников |

**ai-service — новые данные (4 файла):**

| Файл | Назначение |
|---|---|
| `services/ai-service/app/data/customs_offices.json` | IATA → таможенный пост, AWB-префиксы |
| `services/ai-service/app/data/document_codes.json` | Коды документов для графы 44 |
| `services/ai-service/app/data/iata_cities.json` | IATA → название города |
| `services/ai-service/app/data/eu_countries.json` | Список стран ЕС |

**ai-service — изменённые файлы (10 файлов):**

| Файл | Изменения |
|---|---|
| `agent_crew.py` | −3600 строк: удалён мёртвый код, делегация в новые модули |
| `config.py` | +18: embed-настройки (EMBED_PROVIDER, EMBED_MODEL, EMBED_BASE_URL, EMBED_API_KEY), PARSE_PARALLEL_WORKERS |
| `index_manager.py` | +71: Cloud.ru embeddings, очистка описаний, убраны префиксы запросов |
| `dspy_modules.py` | +20: расширены ключевые слова дронов, фильтр RAG score < 0.40 |
| `hs_classifier.py` | +4: расширены ключевые слова приёмников/передатчиков |
| `llm_parser.py` | +17: расширены схемы экстракции tech_description и conformity_declaration |
| `techop_parser.py` | +5: новые поля в промпте (manufacturer, brand, model, article_number, serial_numbers) |
| `invoice_parser.py` | −15: делегация `_safe_float` в `parsing_utils` |
| `llm_client.py` | +2: Cloud.ru `supports_json_mode: True` |
| `smart_parser.py` | +43: очистка описания для classify-hs-rag, множественные suggestions, импорты из новых модулей |
| `workers/tasks.py` | +9: debug-логирование атрибутов товаров при apply-parsed |

**core-api (3 файла):**

| Файл | Изменения |
|---|---|
| `apply_parsed.py` | +38: удаление старых позиций/документов при повторном парсинге, расширение ParsedItem на 5 полей |
| `settings.py` | +48: retry с backoff для RAG-индексации, параметр force, увеличен timeout |
| `models/declaration_item.py` | +2: commercial_name: String(500) → Text |

**Frontend (4 файла):**

| Файл | Изменения |
|---|---|
| `DeclarationFormPage.tsx` | +126: лист дополнения, SourceDrawer: drift warning + RequirementsPanel + RiskPanel + AiExplainPanel |
| `DeclarationStatusPage.tsx` | +97: ДТС кнопки, DocumentUploadPanel, NextActionsPanel, drift warnings, RequirementsPanel, RiskPanel |
| `DeclarationViewPage.tsx` | +74: лист дополнения в печатном формате, pre-line для описания |
| `api/ai.ts` | +2: фильтрация RAG-кандидатов по score ≥ 0.45 |
| `types/index.ts` | +1: serial_number в DeclarationItem |

**Миграции:**

| Файл | Назначение |
|---|---|
| `031_expand_commercial_name_to_text.py` | commercial_name: VARCHAR(500) → TEXT |

**Прочее:**

| Файл | Изменения |
|---|---|
| `docker-compose.yml` | +10: EMBED_PROVIDER/MODEL/BASE_URL/API_KEY для ai-service и ai-worker |
| `docs/architecture.md` | +51: обновлён Этап 2 (ВЫПОЛНЕН), актуализированы ключевые файлы |
